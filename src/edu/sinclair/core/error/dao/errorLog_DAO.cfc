		...
		***********************
		...
		<cffunction name="create" access="public" returntype="void" output="false" displayname="CRUD: Create" hint="I create a new database record for the specified errorLog.">
			...
			<cfscript>
				...
				// event result object
				eventResult = APPLICATION.emsadmin.api.1.objectFactory.getObjectByName("EventResult","bean","event");
				...
				if (variables.instance.useObjectProperties eq 1)
				{
					...
					// Insert the objects
					service = APPLICATION.emsadmin.api.1.objectFactory.getObjectByName("ObjectProperties","service","");
				...
				<cfcatch>
					...
					<cfscript>
						...
						try
						{
							...
							error_service = APPLICATION.emsadmin.api.1.objectFactory.getObjectByName("errorLog","service","error");
							//create errorLog bean to pass through to the service
							errorBean = APPLICATION.emsadmin.api.1.objectFactory.getObjectByName("errorLog","bean","error");			
	...
	***********************
	...
	<cffunction name="loadBean" access="private" returnType="any" output="false">
		...
		<cfscript>
			...
			oBean = APPLICATION.emsadmin.api.1.objectFactory.getObjectByName("errorLog","Bean","error");
