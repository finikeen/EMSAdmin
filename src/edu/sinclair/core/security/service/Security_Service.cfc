...
<!--- ******************************** --->
	<cffunction name="verifyLoginDSN" access="remote" hint="Verifies a user's existence in LDAP" returntype="struct" output="false">
		...
		<cfif isdefined("arguments.username") and len(arguments.password)>
			...
			<!--- If the user was not found via LDAP, check the Security database --->
			<cfif curSubTreeFinal eq "">
				<cfset security = verifyUserSecurityInfo(username=arguments.username, password=arguments.password, verifyPassword=false)>
			...
			<cfif qUser.recordCount>
				...
				<cftry>
					...
					<cfelse>
						...
						<cfset security = verifyUserSecurityInfo(username=arguments.username, password=arguments.password, verifyPassword=true)>
				...
				<cfif qGetUser.recordcount IS 1>
					...
					<cfset security.isStudent = false />
				...
				<cfelse>
					<cfif curSubTreeFinal is "ou=Students,dc=scc-nt,dc=sinclair,dc=edu">
						<cftry>
							<cfldap action="query" server="#ldapserver#" port="#ldapport#" timeout="300"
									start="#curSubTreeFinal#" scope="subtree" attributes="*"
									username="#trim(arguments.username)#" password="#trim(arguments.password)#"
									filter="(sAMAccountName=#trim(arguments.username)#)"
									name="q">
							<cfif q.recordcount>
								...
								<cfset security.user = trim(arguments.username) />
								<cfset security.tartanID = numberformat(right(qGetTartanID.value,7), 0000000) />
								<cfset security.fname = trim(listlast(qGetFullName.value, ',')) />
								<cfset security.lname = trim(listfirst(qGetFullName.value, ',')) />
								<cfset security.pass = passhash />	
								<cfset security.email = trim(qGetEmail.value) />
			                    <cfset security.currentRole = '' />
								<cfset security.currentAccessTypes = arraynew(1) />
								<cfset security.isStudent = true />
								...
								<cfset newclient.roles = structnew() />
								<cfset newclient.roles.current = structnew() />
								<cfset newclient.roles.current.role = 'Student' />
								<cfset newclient.roles.current._access = arraynew(1) />
								<cfset newclient.roles._available = arraynew(1) />
	...
	<cffunction name="verifyUserSecurityInfo" access="remote" hint="Verifies a user's security priviledges in Security database" returntype="struct" output="false">
		...
		<cfif arguments.verifyPassword eq true>
			...
			<cfquery name="qUser" datasource="#variables.securityDSN#">
				...
				WHERE networkUserID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.username#">
				  AND password = <cfqueryparam cfsqltype="cf_sql_varchar" value="#encryptedPass#">
			</cfquery>					
