Ext.define('Adm.util.FormRendererUtils',{	
    ...
    getEvaluationFormItem: function(record,editor){
        ...
        // create the array to hold the main's components
        // so we don't have to add one at a time
        var children = [];
        // create the question's label
        ...
        // create the array to hold the subfields of each question
        var kids = [];
        // default the layout
        var layoutType = 'hbox';
		// list of editable fields
		var editables = ["Field1210","Field1215","Field1216","Field2034"];
        // loop over the subfields
        Ext.each(record.subfields, function(record, index){
            // switch based on the subfield type
			var t = null;
            switch (record.subfieldType) {
                case "shortname":
                case "number":
                case "date":
                case "text":
                case "select":
					var readonly = !Ext.Array.contains(editables, record.id);
					if (!editor) {
						readonly = !editor;
					}
                    t = Ext.create('Ext.form.field.Text', {
                        itemId: record.id,
                        fieldLabel: record.subfieldDesc,
                        labelWidth: 40,
                        margins: '0 0 0 10',
						minWidth: 1,
						readOnly: readonly
                    ...
                    break;
                case "checkbox":
                    t = Ext.create('Ext.form.field.Checkbox', {
                        itemId: record.id,
                        boxLabel: record.subfieldDesc,
						boxLabelCls: 'wrappable-cell-box',
						minWidth: 1,
						editable: false,
						margins: '0 0 0 10'
                    });
                    ...
                    break;
                case "textarea":
                    t = Ext.create('Ext.form.field.TextArea', {
                        itemId: record.id,
                        fieldLabel: record.subfieldDesc,
                        width: 300,
                        margins: '0 0 0 10',
                    ...
                    break;
                /* case "select":
                    t= Ext.create('Ext.form.field.ComboBox', {
						itemId: record.id,
						fieldLabel: record.subfieldDesc,
						width: 240,
						margins: '0 0 0 10',
        ...
        var kidPen = Ext.create('Ext.container.Container', {
            layout: layoutType
        });
        kidPen.add(kids);
        children.push(lbl);
        children.push(kidPen);
        fc.add(children);
        return fc;
    },
	
	setEvaluationFormItems: function(answers) {
		Ext.each(answers, function(record, index){
            var qId = record.questionid;
            var answer = record.answer;
            
            if (answer) {
                var obj = Ext.ComponentQuery.query('#' + qId);
                
                if (!Ext.Object.isEmpty(obj)) {
                    var xType = obj[0].getXType();
                    switch (xType) {
                        case "textfield":
                        case "textareafield":
                            if (record.fieldtype == 'date') {
                                answer = new Date(answer).toDateString();
                            }
                            obj[0].setValue(answer);
                            break;
                        case "combobox":
                            obj[0].setValue(answer);
                            break;
